...
trait SwingApi {
  ...
  implicit class TextFieldOps(field: TextField) {
    ...
    /** Returns a stream of text field values entered in the given text field.
      ...
      */
    def textValues: Observable[String] = Observable.create[String]((observer: Observer[String]) => {

      val r: Reaction = {case ValueChanged(tf) => observer.onNext(field.text)}
  ...
  implicit class ButtonOps(button: Button) {
     ...
     */
    def clicks: Observable[Button] = Observable.create[Button]((observer: Observer[Button]) => {

      val r: Reaction = {case ButtonClicked(tf) => observer.onNext(button)}
