...
trait WikipediaApi {
  ...
  implicit class StringObservableOps(obs: Observable[String]) {
     ...
     */
    def sanitized: Observable[String] = Observable[String](
      observer => {
        obs.subscribe(
          (t: String) => { observer.onNext(t.replace(' ', '_')) },
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
    ...
    /** Given an observable that can possibly be completed with an error, returns a new observable
      ...
      */
    def recovered: Observable[Try[T]] = Observable[Try[T]](
      observer => {
        obs.subscribe(
          (t: T) => {
          ...
          (e: Throwable) => {
            observer.onNext(Failure(e)); observer.onCompleted()
          },
          () => {
            observer.onCompleted()
          }
    ...
    def concatRecovered[S](requestMethod: T => Observable[S]): Observable[Try[S]] = {
      ...
      val yss: Observable[Observable[Try[S]]] = obs map { t => requestMethod(t).recovered}
      yss.concat
