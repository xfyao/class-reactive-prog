...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    // TO IMPLEMENT
    val searchTerms: Observable[String] = Observable.create[String]((observer: Observer[String]) => {

      val r: Reaction = {case ValueChanged(tf) => observer.onNext(searchTermField.text)}

      searchTermField subscribe r

      Subscription {
        searchTermField unsubscribe r
      }
    ...
    val suggestions: Observable[Try[List[String]]] = {
      ...
      searchTerms.flatMap( st => Observable.from[List[String]](Search.wikipediaSuggestion(st)).recovered)
    ...
    val suggestionSubscription: Subscription =  suggestions.observeOn(eventScheduler) subscribe {
      x => x match {
        case Success(s: List[String]) => suggestionList.listData = s
        case Failure(e) => suggestionList.listData = Seq (e.getMessage)
      }
    }

    // TO IMPLEMENT
    val selections: Observable[String] = Observable.create[String]((observer: Observer[String]) => {

      val r: Reaction = {case ButtonClicked(tf) => observer.onNext(suggestionList.selection.items.headOption.getOrElse(searchTermField.text).replace(' ','_'))}
    ...
    val pages: Observable[Try[String]] = {
      ...
      selections.flatMap( st => Observable.from[String](Search.wikipediaPageJson(st)).recovered)
